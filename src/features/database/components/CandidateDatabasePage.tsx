...
const CandidateDatabasePage: React.FC = () => {
    ...
    useEffect(() => {
        const fetchAllUserCandidates = async () => {
            ...
            try {
                ...
                const regularPromise = baserow.get(CANDIDATOS_TABLE_ID, `?user_field_names=true&filter__usuario__link_row_has=${profile.id}`);
                const whatsappPromise = baserow.get(WHATSAPP_CANDIDATOS_TABLE_ID, `?user_field_names=true&filter__usuario__link_row_has=${profile.id}`);
    ...
    const filteredCandidates = useMemo(() => {
        return allCandidates.filter(candidate => {
            ...
            // --- CORREÇÃO APLICADA AQUI: Simplificando a comparação para texto puro ---
