...
interface DashboardPageProps {
  ...
  onDeleteJob: (jobId: number) => Promise<void>; // Ajustado para refletir a função async
...
const DashboardPage: React.FC<DashboardPageProps> = ({
  ...
  const filteredJobs = useMemo(() => {
    const jobsWithStats = jobs.map(job => {
        ...
        let averageScore = 0;
        if (candidateCount > 0) {
            // --- CORREÇÃO APLICADA AQUI: Convertendo o score para Número ---
            // Isso evita a concatenação de strings e calcula a média corretamente.
    ...
    <>
      <div className="fade-in">
        ...
        <RecentScreenings
          ...
          onOpenDeleteModal={handleOpenDeleteModal} // Passando a função para abrir o modal
      ...
      {/* --- RENDERIZANDO O NOVO MODAL DE EXCLUSÃO --- */}
      {jobToDelete && (
        <DeleteJobModal
